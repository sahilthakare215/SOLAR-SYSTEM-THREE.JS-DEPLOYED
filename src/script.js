      ...
      gltf.scene.traverse(function (child) {
          if (child.isMesh) {
              for (let i = 0; i < numberOfAsteroids / 12; i++) {
                  ...
                  scene.add(asteroid);
                  asteroids.push({mesh: asteroid, orbitRadius, angle, speed: THREE.MathUtils.randFloat(0.0005, 0.002)});
              }
          }
      });
  }, undefined, function (error) {
...
// Add meteoroids (small, fast spheres)
function addMeteoroids(count, minOrbit, maxOrbit) {
  for (let i = 0; i < count; i++) {
    const geometry = new THREE.SphereGeometry(THREE.MathUtils.randFloat(0.1, 0.3), 8, 8);
    const material = new THREE.MeshStandardMaterial({ color: 0x888888 });
    const meteoroid = new THREE.Mesh(geometry, material);
    const orbitRadius = THREE.MathUtils.randFloat(minOrbit, maxOrbit);
    const angle = Math.random() * Math.PI * 2;
    ...
    meteoroid.speed = THREE.MathUtils.randFloat(0.002, 0.008);
    meteoroid.orbitRadius = orbitRadius;
    meteoroid.angle = angle;
    scene.add(meteoroid);
    meteoroids.push(meteoroid);
  }
}

// Add comets (elliptical orbits)
function addComets(count) {
  for (let i = 0; i < count; i++) {
    const geometry = new THREE.SphereGeometry(0.5, 16, 16);
    const material = new THREE.MeshStandardMaterial({ color: 0xFFFFFF, emissive: 0x66ccff, emissiveIntensity: 1 });
    const comet = new THREE.Mesh(geometry, material);
    // Elliptical orbit parameters
    comet.a = THREE.MathUtils.randFloat(120, 350); // major axis
    comet.b = THREE.MathUtils.randFloat(40, 120); // minor axis
    comet.angle = Math.random() * Math.PI * 2;
    comet.speed = THREE.MathUtils.randFloat(0.0002, 0.001);
    comet.y = THREE.MathUtils.randFloat(-10, 10);
    scene.add(comet);
    comets.push(comet);
  }
}
...
// Move Pluto closer and make it larger and brighter for visibility
const pluto = new createPlanet('Pluto', 3, 200, 57, plutoTexture)
...
function animate(){
  ...
  //rotating planets around the sun and itself (increased speed)
  sun.rotateY(0.002 * settings.acceleration);
  mercury.planet.rotateY(0.003 * settings.acceleration);
  mercury.planet3d.rotateY(0.008 * settings.accelerationOrbit);
  venus.planet.rotateY(0.0015 * settings.acceleration)
  if (venus.Atmosphere) venus.Atmosphere.rotateY(0.0015 * settings.acceleration);
  venus.planet3d.rotateY(0.0012 * settings.accelerationOrbit);
  earth.planet.rotateY(0.01 * settings.acceleration);
  if (earth.Atmosphere) earth.Atmosphere.rotateY(0.002 * settings.acceleration);
  earth.planet3d.rotateY(0.002 * settings.accelerationOrbit);
  mars.planet.rotateY(0.02 * settings.acceleration);
  mars.planet3d.rotateY(0.0014 * settings.accelerationOrbit);
  jupiter.planet.rotateY(0.01 * settings.acceleration);
  jupiter.planet3d.rotateY(0.0006 * settings.accelerationOrbit);
  saturn.planet.rotateY(0.02 * settings.acceleration);
  saturn.planet3d.rotateY(0.0004 * settings.accelerationOrbit);
  uranus.planet.rotateY(0.01 * settings.acceleration);
  uranus.planet3d.rotateY(0.0002 * settings.accelerationOrbit);
  neptune.planet.rotateY(0.01 * settings.acceleration);
  neptune.planet3d.rotateY(0.00016 * settings.accelerationOrbit);
  pluto.planet.rotateY(0.003 * settings.acceleration);
  pluto.planet3d.rotateY(0.00012 * settings.accelerationOrbit);
...
// Rotate asteroids (faster)
asteroids.forEach(obj => {
  obj.mesh.rotation.y += 0.003;
  obj.angle += obj.speed * settings.accelerationOrbit;
  obj.mesh.position.x = obj.orbitRadius * Math.cos(obj.angle);
  obj.mesh.position.z = obj.orbitRadius * Math.sin(obj.angle);
});

// Animate meteoroids (fast, random orbits)
meteoroids.forEach(meteoroid => {
  meteoroid.angle += meteoroid.speed * settings.accelerationOrbit;
  meteoroid.position.x = meteoroid.orbitRadius * Math.cos(meteoroid.angle);
  meteoroid.position.z = meteoroid.orbitRadius * Math.sin(meteoroid.angle);
});

// Animate comets (elliptical orbits)
comets.forEach(comet => {
  comet.angle += comet.speed * settings.accelerationOrbit;
  comet.position.x = comet.a * Math.cos(comet.angle);
  comet.position.z = comet.b * Math.sin(comet.angle);
  comet.position.y = comet.y;
});
